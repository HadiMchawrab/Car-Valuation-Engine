...
import re
from scraper.maps import body_type_name_to_code, transmission_name_to_code, color_name_to_code
import json
from datetime import datetime



class OpenSooqSpider(Spider):
    name = "opensooq"
    ...
    #     links = response.css('a.postListItemData::attr(href)').getall()
    #     # keep only the ones that go to a “search” page
    #     links = [h for h in links if "/search/" in h]
    #     links = [h.replace("/ar/search/", "/en/search/") for h in links]
    ...
    #   # 2) Grab & parse the NEXT_DATA blob
    #   blob = response.xpath('//script[@id="__NEXT_DATA__"]/text()').get()
    #   if not blob:
    #       self.logger.error("⛔️ __NEXT_DATA__ script not found on page")
    #       return
    #   try:
    #       data = json.loads(blob)
    #   except Exception as e:
    #       self.logger.error("⚠️ Failed to JSON-decode __NEXT_DATA__: %s", e)
    #       return

    #   # 3) Recursively collect all dicts that look like ads
    #   def find_listings(node):
    #       if isinstance(node, dict):
    #           # Heuristic: ad objects always have a numeric postId or id
    #           for key in ("postId", "id", "listingId"):
    #               if key in node and isinstance(node[key], (str, int)):
    #                   yield node
    #                   break
    #           # continue walking
    #           for v in node.values():
    #               yield from find_listings(v)
    #       elif isinstance(node, list):
    #           for item in node:
    #               yield from find_listings(item)

    #   listings = list(find_listings(data))
    #   self.logger.info(f"[Progress] Found {len(listings)} listing objects in JSON blob")

    #   # 4) Enqueue each one
    #   for ad in listings:
    #       ad_id = ad.get("postId") or ad.get("id") or ad.get("listingId")
    #       if not ad_id:
    #           continue
    #       yield response.follow(
    #           f"/en/search/{ad_id}",
    ...
    def parse_ad(self, response):
        ...
        item['body_type'] = body_type_name_to_code.get(raw, '-1')
        ...
        item['transmission_type'] = transmission_name_to_code.get(trans, '-1')
        ...
        item['color']          = color_name_to_code.get(ext_col, '-1')
        item['interior_color'] = color_name_to_code.get(int_col, '-1')
        ...
        item['image_url'] = f"{base_url}{raw_path.lstrip('/').rstrip('.jpg')}.webp"
